...
\begin{figure}[h]
  \includegraphics[width=0.9\textwidth]{Figuras/CSD_Evocada01.pdf}
  \caption{ A comparision of the representations of the displacement of the
    activity. We show several timeframes after stimulus in three representations.
    the leftmost column shows the averaged LFP as measured by the BioCAM.
    In the center column we have the CSD representation, as obtained by
    the kCSD method. In the right column we superimpose over the CSD representation
    the Centers of Mass of each disconnected component, for both sinks and
    sources. In the three columns the saturation of electrodes near
    the point of stimulus is quite evident. Nevertheless, the CSD representation
    provides much better contrast and the patches are better localized as espected.
    The Centers of Mass provide a clear locus of the active subsets of units.
    The area of the circular dots represent the integrated density. The
    bigger ones at the right edge cover the saturated electrodes.
    The anterograde movement of activity follows a high ``sink center'' in the
    first three rows, and then it is followed by a ``source center'', of aproximately
    the same integrated ``mass'', that ends spliting in three smaller components.    
  }\label{lfpcsdcm}
\end{figure}
